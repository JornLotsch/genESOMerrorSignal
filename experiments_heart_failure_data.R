...
transformed_list <- mapply(
  FUN = transform_var,
  x = heart_failure_data,
  method = methods,
  SIMPLIFY = FALSE
)

heart_failure_data_transformed <- as.data.frame(transformed_list)
...
all_boruta_long$color <- ifelse(
  all_boruta_long$Var2 %in% Var_decision$Var[Var_decision$all_boruta.finalDecision == "Confirmed"], "Confirmed",
  ifelse(
